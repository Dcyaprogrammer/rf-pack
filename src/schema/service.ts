...
import type {
  ...
  private apiAdapter: RagflowApiAdapter;

  constructor() {
    this.apiAdapter = new RagflowApiAdapter();
  }
  ...
  private static readonly DATASET_CONFIG: CreateDatasetRequest = {
    ...
    async createDataset() {
      try {
        const response = await this.apiAdapter.createDataset(FinancialReportService.DATASET_CONFIG);
    ...
    async uploadReports(datasetId: string, pdfFiles: File[]) {
      try {
        const response = await this.apiAdapter.uploadDocuments(datasetId, { file: pdfFiles });
    ...
    async parseReports(datasetId: string, documentIds: string[]) {
      try {
        ...
        const response = await this.apiAdapter.parseDocuments(datasetId, parseRequest);
  ...
  请确保回答准确、专业，并基于财报数据提供具体分析。`,
        ...
        const response = await this.apiAdapter.createChatAssistant(assistantConfig);
